...
class RiwayatReviewKemajuanController extends Controller
{
            ...
            if($sesi == 2){
                $riwayats = Usulan::leftJoin('anggota_usulans','anggota_usulans.usulan_id','usulans.id')
                            ...
                            ->select('usulans.id','judul_kegiatan','jenis_kegiatan',
                                    'abstrak','peta_jalan','file_usulan','file_kemajuan','biaya_diusulkan','tahun_usulan','status_usulan','ketua_peneliti_nama as nm_ketua_peneliti',
                                    DB::raw('group_concat(distinct concat(anggota_usulans.anggota_nama) SEPARATOR "<br>") as "nm_anggota" ')
                                    )
                            ->where('reviewer2s.reviewer_nip',Session::get('nip'))
                            ->where('nilai_formulir2s.reviewer_id','!=',null)
                            ->groupBy('usulans.id')
                            ->get();
                return view('reviewer.riwayat_kemajuan.index', compact('riwayats'));
            }
            else{
                Session::flush();

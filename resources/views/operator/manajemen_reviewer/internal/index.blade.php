        ...
        <div class="panel-body" style="border-top: 1px solid #eee; padding:15px; background:white;">
            <div class="row" style="margin-right:-15px; margin-left:-15px;">
                ...
                <div class="col-md-12" style="margin-bottom:5px;">
                    <a href="{{ route('operator.reviewer.add') }}" class="btn btn-primary btn-sm"><i class="fa fa-user-plus"></i>&nbsp; Tambah Reviewer</a>
                </div>
                <div class="col-md-12">
                        ...
                        <tbody>
                            ...
                            @foreach ($reviewers as $reviewer)
                                <tr>
                                    ...
                                    <td> {{ $reviewer->nip }} </td>
                                    <td> {{ $reviewer->nama }} </td>
                                    <td> {{ $reviewer->prodi_nama }} </td>
                                    <td> {{ $reviewer->fakultas_nama }} </td>
                                    <td> {{ $reviewer->nidn }} </td>
                                    ...
                                    <td>
                                        @if ($reviewer->status == "1")
                                            <form action="{{ route('operator.reviewer.nonaktifkan_status',[$reviewer->id]) }}" method="POST">
                                                @csrf @method('PATCH')
                                                <button type="submit" class="btn btn-danger btn sm"><i class="fa fa-thumbs-down"></i></button>
                                            </form>
                                            @else
                                            <form action="{{ route('operator.reviewer.aktifkan_status',[$reviewer->id]) }}" method="POST">
                                                @csrf @method('PATCH')
                                    ...
                                    <td>
                                        <a href="{{ route('operator.reviewer.edit',[$reviewer->id]) }}" class="btn btn-primary btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-edit"></i></a>
                    ...
                    <div class="modal fade" id="modalubahpassword" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                        ...
                        <div class="modal-content">
                            ...
                            <form action=" {{ route('operator.reviewer.update_password') }} " method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-dialog" role="document">
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                            ...
                            <form method="POST" action="{{ route('operator.reviewer.delete') }}">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}

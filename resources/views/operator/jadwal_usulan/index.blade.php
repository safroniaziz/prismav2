...
@section('title', 'Manajemen Jadwal Usulan')
...
@section('content')
    <section class="panel" style="margin-bottom:20px;">
        <header class="panel-heading" style="color: #ffffff;background-color: #074071;border-color: #fff000;border-image: none;border-style: solid solid none;border-width: 4px 0px 0;border-radius: 0;font-size: 14px;font-weight: 700;padding: 15px;">
            <i class="fa fa-user"></i>&nbsp;Manajemen Jadwal Input Usulan
                ...
                <div class="col-md-12">
                    @if ($message = Session::get('success'))
                        ...
                        <div class="alert alert-success alert-block" id="keterangan">
                            <button type="button" class="close" data-dismiss="alert">×</button>
                            <strong><i class="fa fa-info-circle"></i>&nbsp;Perhatian: </strong> Berikut adalah semua data Jadwal Input Usulan yang tersedia, silahkan tambahkan Jadwal Input Usulan baru jika diperlukan !!
                        </div>
                    @endif
                </div>
                <div class="col-md-12" style="margin-bottom:5px;">
                    <button type="button" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#exampleModal">
                        <i class="fa fa-plus" style="font-size:12px;"></i>&nbsp;Tambah Jadwal Input Usulan
                    </button>
                </div>
                <div class="col-md-12">
                        ...
                        <tbody>
                            ...
                            @foreach ($jadwals as $jadwal)
                                <tr>
                                    ...
                                    <td style="text-align:center;">
                                        @if ($jadwal->status == "1")
                                            <form action="{{ route('operator.jadwal_usulan.nonaktifkan_status', [$jadwal->id]) }}" method="POST">
                                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                                <button type="submit" class="btn btn-danger btn-sm" style="color:white; cursor:pointer;"><i class="fa fa-thumbs-down"></i></button>
                                            </form>
                                            @else
                                            <form action="{{ route('operator.jadwal_usulan.aktifkan_status', [$jadwal->id]) }}" method="POST">
                                                {{ csrf_field() }} {{ method_field('PATCH') }}
                                            ...
                                            <div class="modal-dialog" role="document">
                                              <div class="modal-content">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <form method="POST" action="{{ route('operator.jadwal_usulan.delete') }}">
                                                        {{ csrf_field() }}
                                                        {{ method_field('DELETE') }}
                            ...
                            <div class="modal fade" id="exampleModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <form action=" {{ route('operator.jadwal_usulan.add') }} " method="POST">
